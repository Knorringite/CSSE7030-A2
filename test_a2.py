...
from testrunner import AttributeGuesser, OrderedTestCase, TestMaster, skipIfFailed, timeout
...
class TestDeck(TestA2):
    ...
    def test_add_cards(self):
        ...
        deck = self.a2.Deck(self._cards[:])
        deck.add_cards(self._cards[:])
        self.assertEqual(deck.get_cards(), self._cards * 2)
    ...
    # TODO: Check if Card() is fine or better to use other cards
    def test_str(self):
        """ test Deck.__str__ """
        deck = self.a2.Deck([])
...
class TestPlayer(TestA2):
    ...
    @skipIfFailed(TestDeck, TestDeck.test_add_card.__name__)
    ...
    def test_str_cards(self):
        """ test Player.__str__ with cards in deck """
        p = self.a2.Player("Steven")
        p.get_hand().add_card(self.a2.Card())
        self.assertEqual(str(p), 'Player(Steven, Deck(Card()), Deck())')

        p.get_coders().add_card(self.a2.Card())
        self.assertEqual(str(p), 'Player(Steven, Deck(Card()), Deck(Card()))')
    ...
    def test_repr_cards(self):
        ...
        p = self.a2.Player("Steven")
        p.get_hand().add_card(self.a2.Card())
        self.assertEqual(repr(p), 'Player(Steven, Deck(Card()), Deck())')

        p.get_coders().add_card(self.a2.Card())
        self.assertEqual(repr(p), 'Player(Steven, Deck(Card()), Deck(Card()))')


@skipIfFailed(TestDesign, TestDesign.test_classes_defined.__name__, 'Player')
@skipIfFailed(TestDesign, TestDesign.test_classes_defined.__name__, 'Deck')
@timeout(5)  # Just in case
class TestPlayAndAction(TestA2):
    def setUp(self):
        if self.a2_support is None:
            self.skipTest("Failed to import 'a2_support.py'")

    def init_game(self, card_type, *card_args, size=5, num_players=3, cycle=True):
    ...
    # TODO: Include test for get_sleeping_coder None check?
    ...
    # TODO: Include test for get_sleeping_coder None check?
    # TODO: Include test for targeting self (same player)?
    ...
    def test_all_nighter_action(self):
        ...
        self.assertIs(coder, new_state.coders[slot])
